...
def train(net,data_iter_train,data_iter_test,num_epochs,lr,step_size = 5,gamma = 0.1):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if test_loss/num_test < best_loss:
                ...
                path = './surr_model/total_'+str(num_epochs)+'_lr_'+str(lr)+'_epoch_'+str(epoch)+'_moduli_cal_1_channel.pt'
                torch.save(net,path)
    ...
    path = './surr_model/total_'+str(num_epochs)+'_lr_'+str(lr)+'_moduli_cal_1_channel.pt'
    torch.save(net,path)
...
net = Dis_res_3d(channels=1).to(device)
