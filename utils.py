...
def elemsplot(size_ele, x_cor, y_cor, z_cor, num_plots,id):
    index_border = x_cor.size()[0]
    stru_sel = random.sample(range(int(index_border)), num_plots)
    for i in stru_sel:
        # build up structure
        n_voxels = torch.zeros((4, 4, 4), dtype=bool)
        index = (
            (x_cor[i]-1).long(),
            (y_cor[i]-1).long(),
            (z_cor[i]-1).long(),
        )
        reinfor = torch.ones((32), dtype=bool)
        n_voxels.index_put_(index, reinfor)
        facecolors = np.where(n_voxels, '#EE4C2C', '#812CE5')
        edgecolors = np.where(n_voxels, None, None)
        ...
        save_path = './image_res_epoch_100/figure_epoch_'+str(id)+'_image_'+str(i)+'.png'
...
def transfer_symmetry_1_8(x_out):
    x_out_1 = x_out.clone()
    x_out_2 = x_out.clone()
    x_out_2 = torch.flip(x_out_2,[4])
    x_temp_semi_1 = torch.cat((x_out_1,x_out_2),dim=4)
    x_out_3 = x_out.clone()
    x_out_3 = torch.flip(x_out_3,[3])
    x_out_4 = x_out.clone()
    x_out_4 = torch.flip(x_out_4,[3,4])
    x_temp_semi_2 = torch.cat((x_out_3,x_out_4),dim=4)
    x_temp_half_1 = torch.cat((x_temp_semi_1,x_temp_semi_2),dim=3)
    x_out_5 = x_out.clone()
    x_out_5 = torch.flip(x_out_5,[2])
    x_out_6 = x_out.clone()
    x_out_6 = torch.flip(x_out_6,[2,4])
    x_temp_semi_3 = torch.cat((x_out_5,x_out_6),dim=4)
    x_out_7 = x_out.clone()
    x_out_7 = torch.flip(x_out_7,[2,3])
    x_out_8 = x_out.clone()
    x_out_8 = torch.flip(x_out_8,[2,3,4])
    x_temp_semi_4 = torch.cat((x_out_7,x_out_8),dim=4)
    x_temp_half_2 = torch.cat((x_temp_semi_3,x_temp_semi_4),dim=3)
    x_total = torch.cat((x_temp_half_1,x_temp_half_2),dim=2)
    return x_total

def transfer_symmetry_1_4(x_out):
    x_out_1 = x_out.copy_()
    x_out_2 = x_out.copy_()
    x_out_2 = torch.flip(x_out_2,[4])
    x_temp_half_1 = torch.cat(x_out_1,x_out_2,dim=4)
    x_out_3 = x_out.copy_()
    x_out_3 = torch.flip(x_out_3,[3])
    x_out_4 = x_out.copy_()
    x_out_4 = torch.flip(x_out_4,[3,4])
    x_temp_half_2 = torch.cat(x_out_3,x_out_4,dim=4)
    x_total = torch.cat(x_temp_half_1,x_temp_half_2,dim=3)
    return x_total

def transfer_symmetry_1_2(x_out):
    x_out_1 = x_out.copy_()
    x_out_2 = x_out.copy_()
    x_out_2 = torch.flip(x_out_2,[4])
    x_total = torch.cat(x_out_1,x_out_2,dim=4)
    return x_total
...
def transfer_3d_to_channels_2_for_batch(x_out):
    x_out = x_out.cuda()
    x_inv = torch.zeros((x_out.size()),dtype=torch.float).cuda()
    x_inv[x_out==0] = 1
    x_out = torch.unsqueeze(x_out,dim=1)
    x_inv = torch.unsqueeze(x_inv,dim=1)
    out = torch.cat((x_inv,x_out),dim=1)
    return out


def transfer_3d_gan_out_max(x_out):
    x_out = x_out.transpose(1,2).transpose(2,3).transpose(3,4)
    idx = torch.max(x_out,dim=4)
    sum_matrix = torch.zeros((x_out.size(0),)).cuda()
    for i in range(x_out.size(0)):
        sum_matrix[i] = torch.sum(idx.indices[i,:,:,:])
    return idx.indices,sum_matrix

if __name__ == '__main__':
    x_out = torch.rand((8,2,6,6,6)).cuda()
    out = transfer_3d_gan_out_limit(x_out)
    #print(x_out)
    #print(out)
