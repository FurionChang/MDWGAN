...
class WGAN_GP(object):
    def __init__(self,G_path,D_path,batch_size,lr=1e-4,b1=0.5,b2=0.999,G_channels=100,D_channels=2,surr_path=None):
        self.G_path = G_path
        self.D_path = D_path
        ...
        if surr_path is not None:
            self.moduli_cal = torch.load(surr_path).cuda()
        else:
            # Use pretrained surrogate model
            if D_channels == 2:
                self.moduli_cal = torch.load('./model/surr_model/2_channel_model/2_channel_surr_model.pt').cuda()
            elif D_channels == 1:
                self.moduli_cal = torch.load('./model/surr_model/1_channel_model/1_channel_surr_model.pt').cuda()
        ...
        self.generator_iters = int(20000/self.batch_size)
        self.critic_iter = 3
        self.lambda_term = 10
    ...
    def train(self, train_loader,obj,alpha=30,beta=30,epochs=100):
        #self.t_begin = t.time()
        ...
        for p in self.moduli_cal.parameters():
        ...
        for epoch in range(epochs):
            ...
            for g_iter in g_iter_tqdm:
                ...
                # Loss G2: moduli loss
                if obj == 'lg' or obj =='all':
                    z = self.get_torch_variable(torch.randn(self.batch_size, 100, 1, 1, 1))
                    fake_images = self.G(z)
                    m_loss_goal = torch.ones((self.batch_size,3),dtype = torch.float).cuda()
                    m_loss_goal += 0.9
                    out = self.moduli_cal(fake_images)
                    m_loss = alpha*self.moduli_critic(out,m_loss_goal)
                    m_loss.backward()
                
                # Loss G3: iso loss
                if obj == 'iso' or obj == 'all':
                    z = self.get_torch_variable(torch.randn(self.batch_size, 100, 1, 1, 1))
                    fake_images = self.G(z)
                    s_loss = self.moduli_cal(fake_images)
                    s_loss = beta*self.calculate_moduli_symm_loss(s_loss)
                    s_loss.backward()
    ...
    def generate_samples(self,num,path=None):
        ...
        out_samples = torch.stack(out_samples,dim=0)
        if path is not None:
            torch.save(out_samples,path)
        return out_samples
